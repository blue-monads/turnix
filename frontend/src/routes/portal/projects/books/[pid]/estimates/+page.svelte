<script lang="ts">
    ...
    import { NewBookAPI, type Estimate, type Sale } from "$lib/projects/books";
    ...
    const load = async () => {
        loading = true;
        const resp = await api.listEstimate(pid);
        if (resp.status !== 200) {
            return;
        }

        estimates = (resp.data || []).map((item) => {
            return {
                ...item,
...
<AppBar>
    ...
    <svelte:fragment slot="trail">
        ...
        <a
            href={`/z/pages/portal/projects/books/${pid}/estimates/new`}
        ...
        actions={[
            {
                ...
                Action: async (id) => {
                    goto(
                        `/z/pages/portal/projects/books/${pid}/estimates/preview?sid=${id}`,
                    );
                },
            },
